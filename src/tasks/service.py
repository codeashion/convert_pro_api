...
def create_task(db: Session, token: str, task_data: TaskCreate) -> TaskOut:
    ...
    try:
        ...
        # Only create the first task (today's entry)
        new_task = Task(
            user_id=user_id,
        ...
        # If reminder_enabled, create a reminder for today
        ...
        # Prepare response for the created task
        assignments = db.execute(
            select(TaskAssignment, FamilyMember)
            .join(FamilyMember, TaskAssignment.family_member_id == FamilyMember.id)
            .where(TaskAssignment.task_id == new_task.id)
        ).all()
