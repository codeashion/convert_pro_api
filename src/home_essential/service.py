...
def get_all_items(db: Session, token: str):
    """Get all home essential items for logged-in user"""
    token_data = verify_token(token)
    user_id = int(token_data.get("id"))

    return db.execute(
        select(HomeEssential).where(HomeEssential.user_id == user_id)
    ).scalars().all()


def get_item_by_id(db: Session, item_id: int, token: str):
    """Get a specific home essential item by ID for the logged-in user"""
    token_data = verify_token(token)
    user_id = int(token_data.get("id"))

    item = db.execute(
        select(HomeEssential).where(and_(HomeEssential.id == item_id, HomeEssential.user_id == user_id))
    ).scalars().first()
...
def add_item(db: Session, token: str, data: HomeEssentialCreate):
    """Add a new home essential item for the logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        existing = db.execute(
            select(HomeEssential).where(
                and_(HomeEssential.name == data.name, HomeEssential.user_id == user_id)
            )
        ).scalars().first()
        ...
        new_item = HomeEssential(name=data.name, status=data.status, user_id=user_id)
        db.add(new_item)
        db.commit()
        db.refresh(new_item)
...
def update_item(db: Session, item_id: int, data: HomeEssentialCreate, token: str):
    """Update an existing home essential item for the logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        existing = db.execute(
            select(HomeEssential).where(
                and_(HomeEssential.name == data.name, HomeEssential.user_id == user_id, HomeEssential.id != item_id)
            )
        ).scalars().first()
...
def delete_item(db: Session, item_id: int, token: str):
    """Delete a home essential item for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        return {"statusCode": 200, "status": True, "message": "Home essential item deleted successfully"}
...
def delete_true_home_essentials(db: Session, token: str):
    """Delete all home essential items with status=True for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))

        items = db.execute(
            select(HomeEssential).where(and_(HomeEssential.status == True, HomeEssential.user_id == user_id))
        ).scalars().all()
...
def get_all_groceries(db: Session, token: str):
    """Get all grocery items for logged-in user"""
    token_data = verify_token(token)
    user_id = int(token_data.get("id"))

    return db.execute(
        select(Grocery).where(Grocery.user_id == user_id)
    ).scalars().all()


def get_grocery_by_id(db: Session, grocery_id: int, token: str):
    """Get a specific grocery item by ID for logged-in user"""
    token_data = verify_token(token)
    user_id = int(token_data.get("id"))

    grocery = db.execute(
        select(Grocery).where(and_(Grocery.id == grocery_id, Grocery.user_id == user_id))
    ).scalars().first()
...
def add_grocery(db: Session, token: str, data: GroceryCreate):
    """Add a new grocery item for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        existing = db.execute(
            select(Grocery).where(and_(Grocery.name == data.name, Grocery.user_id == user_id))
        ).scalars().first()

        if existing:
            raise HTTPException(status_code=400, detail="Grocery item with this name already exists")

        new_grocery = Grocery(name=data.name, status=data.status, user_id=user_id)
        db.add(new_grocery)
        db.commit()
        db.refresh(new_grocery)
...
def update_grocery(db: Session, grocery_id: int, data: GroceryCreate, token: str):
    """Update an existing grocery item for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        existing = db.execute(
            select(Grocery).where(
                and_(Grocery.name == data.name, Grocery.user_id == user_id, Grocery.id != grocery_id)
            )
        ).scalars().first()
...
def delete_grocery(db: Session, grocery_id: int, token: str):
    """Delete a grocery item for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))
        ...
        return {"statusCode": 200, "status": True, "message": "Grocery item deleted successfully"}
...
def delete_true_groceries(db: Session, token: str):
    """Delete all groceries where status=True for logged-in user"""
    try:
        token_data = verify_token(token)
        user_id = int(token_data.get("id"))

        items = db.execute(
            select(Grocery).where(and_(Grocery.status == True, Grocery.user_id == user_id))
        ).scalars().all()
